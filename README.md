...
这个仓库当前更接近“本地插件仓库 / bot 工程内插件目录”，不是标准的 PyPI 发布仓库。

如果你要直接使用当前代码，推荐方式是：

1. 将 `nonebot_plugin_ai_groupmate/` 放进你自己的 NoneBot 插件目录
2. 在 bot 工程内安装依赖并加载插件
3. 配置环境变量
4. 执行 ORM 迁移

如果你要用标准包管理、测试、CI、PyPI 发布流程，优先参考原项目。

## 💿 接入方式

### 1. 放入插件目录

将当前仓库中的 `nonebot_plugin_ai_groupmate/` 目录放入你的 bot 项目插件目录，例如：
...
- 原项目更偏“标准插件发布版”；当前仓库更偏“本地插件仓库 / 自用部署版”
...
- 当前 README 以当前仓库代码和当前分支行为为准
- 如果你要对齐原项目的发布形态，请回原项目 README 和 `pyproject.toml`
